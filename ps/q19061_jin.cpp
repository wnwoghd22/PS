...
void sweep_arcs(std::vector<Arc>& arcs, Polygon& hull, int max_idx, ld alpha) {
	std::rotate(hull.begin(), hull.begin() + max_idx, hull.end());
	std::vector<ArcInt> stack;
	Pos pivot = hull[0];
	for (const Pos& p : hull) {
		ld l = rad(pivot, p);
		ld r = l + alpha;
		ld dist = p.Euc();
		while (stack.size()) {
			if (stack.back().r < l) break;
			else {
				if (stack.back().l >= l) {
					if (dist >= stack.back().euc) stack.pop_back();
					else {
						l = stack.back().r;
						break;
					}
					...
					if (dist >= stack.back().euc) stack.back().r = l;
					else l = stack.back().r;
					break;
				}
			}
		}
		stack.push_back({ p.Euc(), l, r });
	}
	stack.back().r = std::min(stack.back().r, 2 * PI);
}
...
ld query() {
	...
	for (int i = 0; i < N; i++) {
		...
		v = p.rot(A);
		H[i] = p, V[i] = v;
		if (p.mag() > H[max_idx].mag()) max_idx = i;
	}
