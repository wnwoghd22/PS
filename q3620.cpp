...
int main() {
	...
	for (int i = 0, j = 1; j < N; ++i, ++j) {
		dx = pos[i].x - pos[j].x;
		dy = pos[j].y - pos[i].y;
		c = dy * pos[i].x + dx * pos[i].y - hypot(dx, dy) * R;
		lines[i] = { { dx, dy }, c };
	}
	dx = pos[N - 1].x - pos[0].x;
	dy = pos[0].y - pos[N - 1].y;
	c = dy * pos[0].x + dx * pos[0].y - hypot(dx, dy) * R;
	lines[N - 1] = { { dx, dy }, c };
	std::sort(lines, lines + N);
	half_plane_intersection(N);

	int ans[2];
	ld max_dist = 0, cur_dist;
	for (int i = 0; i < N; ++i) {
		// std::cout << hull[i].x << ' ' << hull[i].y << '\n';
		for (int j = 0; j < N; ++j) {
			if (j == i) continue;
			if ((cur_dist = dist(hull[i], hull[j])) > max_dist) {
				max_dist = cur_dist;
				ans[0] = i; ans[1] = j;
			}
		}
	}
	std::cout << hull[ans[0]].x << ' ' << hull[ans[0]].y
		<< ' ' << hull[ans[1]].x << ' ' << hull[ans[1]].y;
}