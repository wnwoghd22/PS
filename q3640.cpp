...
void solve(int v, int e) {
	Network network(v * 2 + 2);
	network.add_edge(2 * v + 1, v + 1, 2, 0);
	for (int i = 1; i <= v; ++i) network.add_edge(i, v + i, 1, 0);
	while (e--) {
		std::cin >> a >> b >> c;
		network.add_edge(v + a, b, 1, c);
	}
	std::cout << network.MCMF(2 * v + 1, v).first << '\n';
}
